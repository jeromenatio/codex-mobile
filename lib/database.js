...
function createDatabase(file, defaultWorkspaceRoot, legacyStateFile = "") {
  ...
  return {
    ...
    searchSessions(query, workspaceRoot) {
      return searchSessions(db, query, workspaceRoot);
    },
...
function seedDefaults(db, defaultWorkspaceRoot) {
  ...
  setAppConfig.run("telegram_config", JSON.stringify(defaultTelegramConfig()));
...
function loadState(db, defaultWorkspaceRoot) {
  ...
  const telegramConfig = normalizeTelegramConfig(
    safeJsonParse(db.prepare("SELECT value FROM app_config WHERE key = 'telegram_config'").get()?.value, {})
  );
...
function searchSessions(db, query, workspaceRoot) {
  ...
  const termClauses = terms.map((term) => {
    ...
    return `(
      ...
      OR lower(s.workspace_name) LIKE ? ESCAPE '\\'
      OR EXISTS (
        SELECT 1
        FROM messages m
  ...
  const rows = db.prepare(`
    SELECT s.id, s.name, s.workspace_id, s.workspace_name, s.workspace_path, s.workspace_root,
           s.created_at, s.updated_at, s.status, s.thread_id, s.rollout_path,
  ...
  return rows.map((row) => ({
    ...
    threadId: row.thread_id || null,
    rolloutPath: row.rollout_path || null,
    messages: new Array(Number(row.message_count || 0)),
  }));
}
...
function saveState(db, state, defaultWorkspaceRoot) {
  ...
  `);
  const upsertTelegramConfig = db.prepare(`
    INSERT INTO app_config(key, value) VALUES ('telegram_config', ?)
    ON CONFLICT(key) DO UPDATE SET value = excluded.value
  `);
  ...
  runTransaction(db, () => {
    ...
    upsertTelegramConfig.run(JSON.stringify(normalized.appConfig.telegram));
    upsertUiState.run(
...
function normalizeState(state, defaultWorkspaceRoot) {
  ...
  return {
    ...
    appConfig: {
      workspaceRoot,
      telegram: normalizeTelegramConfig(state?.appConfig?.telegram),
    },
    uiState: {
...
function normalizeTelegramConfig(config) {
  ...
  return {
    ...
    botTokenSecretKey: normalizeSecretKey(config?.botTokenSecretKey) || defaults.botTokenSecretKey,
    allowedChatIds: String(config?.allowedChatIds || "").trim(),
    workspace: slugify(String(config?.workspace || defaults.workspace)) || defaults.workspace,
  };
}
